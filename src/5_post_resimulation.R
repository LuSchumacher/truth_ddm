...
fit_exp_2_rt_total <- as_draws_matrix(fit_exp_2_rt_total)
...
pred_data_exp1 <- bind_rows(
  pred_data_session_1, pred_data_session_2
)
emp_data_exp1 <- bind_rows(df_session_1, df_session_2)
...
pred_resp_prob_exp2 <- pred_data_exp_2 %>% 
  group_by(id, stim_type) %>% 
  summarise(resp_prob = mean(resp), .groups = "drop") %>% 
  group_by(stim_type) %>% 
  summarise(
...
group_emp_resp_prob_exp2 <- df_exp_2 %>% 
  group_by(id, stim_type) %>% 
  summarise(resp_prob = mean(resp), .groups = "drop") %>% 
  group_by(stim_type) %>% 
  summarise(
