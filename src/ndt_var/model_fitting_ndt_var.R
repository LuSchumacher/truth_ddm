...
init_fun <- function(chains = 4){
  ...
  for (i in 1:chains) {
    L[[i]] = list(
      ...
      mu_ndt      = 0.5 + runif(4, -0.1, 0.1),
      sigma_ndt   = 0.05 + runif(4, -0.05, 0.05),
      z_v         = matrix(runif(4*N, -1, 1), nrow=4),
      z_a         = matrix(runif(4*N, -1, 1), nrow=4),
      z_ndt       = matrix(runif(4*N, -0.15, 0.15), nrow=4),
      z_bias      = matrix(runif(4*N, -1, 1), nrow=4),
      trel        = matrix(runif(4*N, 0.01, 0.99), nrow=4),
      s           = runif(`T`, 0, 0.05)
    )
...
fit_session_1$save_object("../../fits/fit_session_1_ndt_var.rds")
...
mcmc_trace(fit_session_2$draws(inc_warmup = TRUE), n_warmup = 3000, pars=PARAM_NAMES)

fit_session_2$save_object("../../fits/fit_session_2_ndt_var.rds")
...
mcmc_trace(fit_exp_2$draws(inc_warmup = TRUE), n_warmup = 3000, pars=PARAM_NAMES)

fit_exp_2$save_object("../fits/fit_exp_2_ndt_var.rds")
