...
fit_session_1 <- readRDS("../fits/fit_session_1_regression_approach_v2")
...
# Extract and reshape
draws <- fit_session_1$draws(variables = PARAM_NAMES)
draws_df <- as_draws_df(draws)

draws_long <- draws_df %>%
  pivot_longer(cols = all_of(PARAM_NAMES), names_to = "parameter", values_to = "value") %>%
  mutate(
    row_raw = str_extract(parameter, "^[a-z_]+(?=_betas)"),
    beta_index = str_extract(parameter, "\\[\\d\\]") %>% str_remove_all("\\[|\\]"),
    col = case_when(
      beta_index == "1" ~ "Repetition status",
      beta_index == "2" ~ "Factual truth",
    ...
    row_label = case_when(
      ...
      row_raw == "bias"     ~ "bias",
      row_raw == "ndt"      ~ "NDT",
      row_raw == "ndt_var"  ~ "NDT var"
    ),
    col = factor(col, levels = c("Repetition status", "Factual truth", "Interaction")),
    row_label = factor(row_label, levels = c("v", "a", "bias", "NDT", "NDT var"))
  )

ggplot(draws_long, aes(x = value)) +
  geom_density(fill = COLOR_PALETTE[1], alpha = 0.6) +
  facet_grid(rows = vars(row_label), cols = vars(col), scales = "free", labeller = label_value) +
...
# Compute full condition difference
diff_full <- 2 * draws_long$value[draws_long$parameter == "ndt_var_betas[1]"]

# Posterior density at 0
posterior_density <- density(diff_full)
posterior_density_at_0 <- posterior_density$y[which.min(abs(posterior_density$x - 0))]

# Prior density at 0 — use sd = 1.0
prior_density_at_0 <- dnorm(0, mean = 0, sd = 1.0)

# Bayes Factor: null over alternative
