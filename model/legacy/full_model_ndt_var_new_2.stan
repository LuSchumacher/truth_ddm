...
transformed parameters {
  ...
  vector<lower=0>[4] s_ndt_s;
  
  s_v     = softmax(sigma_v);
  s_a     = softmax(sigma_a);
  s_ndt   = softmax(sigma_ndt);
  s_bias  = softmax(sigma_bias);
  s_ndt_s = softmax(sigma_ndt_s);
    ...
    ndt_s[i] = softplus(mu_ndt_s[i] + s_ndt_s[i] * z_ndt_s[i,]);
	}
}
...
model {
  ...
  for (i in 1:4) {
  	...
  	z_ndt_s[i] ~ std_normal();
  }

  for (i in 1:T) {
    t0[i] = ndt[condition[i], subject_id[i]] + s[i] * ndt_s[condition[i], subject_id[i]];
  }  
...
generated quantities {
  ...
  for (i in 1:4) {
    ...
    transf_mu_ndt_s[i] = softplus_r(mu_ndt_s[i]);
  }
}
