functions {
  real partial_sum_fullddm(
    ...
    matrix ndt, real ndt_s, array[] int subject_id, array[] int resp, array[] int condition
  ) {
    real ans = 0;
    for (i in start:end) {
      if (resp[i] == 1) {
        ans += wiener_lpdf(
          ...
          v[condition[i], subject_id[i]], 0, 0, ndt_s
        );
      } else {
        ans += wiener_lpdf(
          rt_slice[i+1-start] | a[condition[i], subject_id[i]],
...
model {
  ...
  target += reduce_sum(
    ...
    a, v, bias, ndt, ndt_s, subject_id, resp, condition
  );
}
